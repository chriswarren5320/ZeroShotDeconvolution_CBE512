...
from utils.loss2 import create_psf_loss,cal_psf_2d,psf_estimator_2d
...
# about data
parser.add_argument("--otf_or_psf_path", type=str, default="E:\\Christopher\\ZSD\\train_inference_python\\20251115\\PSF.tif")
parser.add_argument("--psf_src_mode", type=int, default='1') # 1:psf in .tif format; 2:otf in .mrc format
parser.add_argument("--dxypsf", type=float, default=0.0313) # dxy of simulated PSF
parser.add_argument("--dx", type=float, default=0.0626)
parser.add_argument("--dy", type=float, default=0.0626)
parser.add_argument("--data_dir", type=str, default="E:\\Christopher\\ZSD\\train_inference_python\\20251115\\data\\")
parser.add_argument("--folder", type=str, default='bg_700_std_50_SegNum400')
parser.add_argument("--test_images_path", type=str, default="E:\\Christopher\\ZSD\\train_inference_python\\20251115\\data\\bg_700_std_10_SegNum400\\input\\cell01_reco1_00000004.tif")
parser.add_argument("--save_weights_dir", type=str, default='E:\\Christopher\ZSD\\your_saved_models\\')
parser.add_argument("--save_weights_suffix", type=str, default="_TV0.02")
...
parser.add_argument("--TV_rate", type=float, default=0.02)
parser.add_argument("--Hess_rate", type=float, default=0) #0.02
parser.add_argument("--Energy_rate", type=float, default=0) #0.02
parser.add_argument("--Adapt_rate", type=float, default=0) #0.02
parser.add_argument("--Wavelet_rate", type=float, default=0) #0.02
parser.add_argument("--Sparse_rate", type=float, default=0) #0.02
parser.add_argument("--adapt_k", type=float, default=0.1)
...
with open(save_weights_path+"config.txt","a", encoding="utf-8") as f:
    f.write('\n\n')
    f.write(str(args))
...
loss = create_psf_loss(psf_g, TV_weight=TV_rate, Hess_weight=Hess_rate, laplace_weight=0,
                    l1_rate=l1_rate, mse_flag=mse_flag,  
                    upsample_flag=upsample_flag, insert_xy=insert_xy, deconv_flag=1,
                    energy_weight=Energy_rate, 
                    adapt_weight=Adapt_rate, adapt_type='inv', adapt_eps=1e-6, adapt_k=adapt_k,
                    wavelet_weight=Wavelet_rate, wavelet_levels=1, wavelet_type='haar', wavelet_penalty='l1',
                    sparse_weight=Sparse_rate, sparse_type='l1_grad', sparse_eps=1e-6)
...
#     print("UnicodeDecodeError when creating writer:", e)
#     print("TensorBoard writer failed. Using fallback empty in-memory writer.")
#     # fallback: create a new unique folder with timestamp to avoid old files
#     log_path = os.path.join(save_weights_path, 'graph_fallback_' + datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
...
for it in range(iterations-load_init_model_iter):
    ...
    if it % 50 == 0:
        print("%d it: time: %s, denoise_loss = %.3e, deconv_loss = %.3e" % (it + 1 + load_init_model_iter, elapsed_time, 
                                                                            loss_generator[1], loss_generator[2]))
